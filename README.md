...
```text
codex-quota-monitor v0.8.1
CodexQuotaMenu v0.8.1 (build 14)
```
...
官方 5 小时额度或周额度从未使用时，重置时间可能随查询继续向后移动。插件在启动首次刷新、手动刷新和每 5 分钟自动刷新成功后都会检查官方剩余百分比；任一窗口为 100% 时，通过 `codex exec` 发起一次要求只回复 `OK` 的真实请求。该请求使用只读沙箱，不调用工具，也不使用额度重置券。
