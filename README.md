...
> 适用范围：本手册按用户 `xxxxxx` 当前这台 Mac 的实际安装路径编写。插件已安装在个人 marketplace `personal` 中。
...
```text
/Users/xxxxxx/plugins/codex-quota-monitor
```

### 3.1 启动菜单栏

```bash
python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/launch_menu_bar.py
```
...
```bash
python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/launch_menu_bar.py --stop
```
...
```bash
python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/launch_menu_bar.py --stop
python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/launch_menu_bar.py --rebuild
```
...
```bash
python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/codex_quota.py
```

### 3.6 查看机器可读的 JSON

```bash
python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/codex_quota.py --json
```

默认会遮住部分邮箱地址。如果明确需要显示完整邮箱，可运行：

```bash
python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/codex_quota.py --show-email
```
...
| 操作 | 命令 |
|---|---|
| 启动 | `python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/launch_menu_bar.py` |
| 停止 | `python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/launch_menu_bar.py --stop` |
| 重启 | 先运行停止命令，再运行启动命令 |
| 强制重建并重启 | 先运行停止命令，再运行带 `--rebuild` 的启动命令 |
| 查看额度 | `python3 /Users/xxxxxx/plugins/codex-quota-monitor/scripts/codex_quota.py` |
