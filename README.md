...
启动首次刷新、点击按钮手动刷新和每 5 分钟自动补查，都会执行额度窗口维持检查。官方返回 5 小时额度时，只有该额度为 100% 才会发起一次真实、最小化的 Codex 请求；即使周额度为 100%，只要 5 小时额度不是 100% 就不会执行。仅当官方没有返回 5 小时窗口时，才回退检查周额度。最小请求成功后，5 小时窗口至少冷却 5 小时，周额度回退至少冷却 7 天；请求失败时最早 15 分钟后重试。
...
```text
codex-quota-monitor v0.8.3
CodexQuotaMenu v0.8.3 (build 16)
```
...
官方额度窗口从未使用时，重置时间可能随查询继续向后移动。插件在启动首次刷新、手动刷新和每 5 分钟自动刷新成功后都会检查官方剩余百分比。官方返回 5 小时额度时，只以该窗口为触发条件：5 小时额度为 100% 才执行最小请求；即使周额度为 100%，只要 5 小时额度不是 100% 就不会执行。仅当官方完全没有返回 5 小时窗口时，才回退到周额度 100% 作为触发条件。

触发后，插件通过 `codex exec --model gpt-5.5` 发起一次要求只回复 `OK` 的真实请求。该请求固定使用 GPT-5.5、只读沙箱，不调用工具，也不使用额度重置券。

最小请求成功后，5 小时窗口至少冷却 5 小时，周额度回退至少冷却 7 天，并在官方未来的重置时间更晚时延长到该时间。即使接口暂时返回过期或缺失的 `resetsAt`，或最小请求太小导致官方整数百分比仍显示 100%，连续刷新也不会在同一窗口重复请求。只有请求失败时才使用 15 分钟重试冷却。

请求完成后产生的临时任务会自动归档，但其数值 Token 事件仍计入“今日 Tokens”。插件只把触发时间、原因和每个窗口下次允许触发的时间保存到 `~/.codex/codex-quota-monitor/quota-keepalive.json`，不记录请求内容、回复内容或认证信息。需要临时关闭这一功能时，可在启动菜单栏程序前设置环境变量 `CODEX_QUOTA_KEEPALIVE=0`。

最小 Token 请求确认成功后，菜单栏应用会弹出 macOS 系统提醒：“检测到额度为 100%，已执行一次最小 Token 消耗以固定重置时间。”首次触发时 macOS 可能询问通知权限，请选择“允许”。处于冷却期、无需触发或请求失败时不会发送成功提醒。
