...
启动首次刷新、点击按钮手动刷新和每 5 分钟自动补查，都会执行额度窗口维持检查：如果 5 小时额度或周额度的剩余值为 100%，插件会发起一次真实、最小化的 Codex 请求，让尚未使用的滚动窗口开始计时。两个窗口同时为 100% 时也只发起一次；每个窗口会分别冷却到自己的重置时间，因此连续刷新不会重复消耗，请求失败时最早 15 分钟后重试。
...
```text
codex-quota-monitor v0.8.2
CodexQuotaMenu v0.8.2 (build 15)
```
