...
打开额度窗口，点击右上角的“刷新”按钮。程序也会每 5 分钟自动刷新，周额度倒计时每 30 秒更新一次。
...
首次运行且没有缓存时，所有缺失的官方数据均显示“暂无数据”，不会显示为 0。缓存位于 `~/.codex/codex-quota-monitor/official-usage-cache.json`，只保存每日 Token 数字和成功获取时间，不保存登录令牌。
